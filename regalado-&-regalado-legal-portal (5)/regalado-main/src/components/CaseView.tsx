...
const statusStyles: { [key in CaseStatus]: string } = {
    ...
    [CaseStatus.Closed]: 'bg-zinc-700 text-zinc-100',
};

const CaseCard: React.FC<{ caseData: Case }> = ({ caseData }) => {
    return (
        <div className="bg-black border border-zinc-800 p-6 shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between">
            <div>
                ...
                <p className="text-zinc-400 mb-1 font-mono text-sm">Caso ID: {caseData.id.substring(0, 8)}...</p>
                <p className="text-zinc-300 mt-4">{caseData.descripcion}</p>
            </div>
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <button 
                    onClick={fetchCases} 
                    disabled={loading} 
                    className="flex items-center bg-zinc-800 hover:bg-zinc-700 text-white font-bold py-2 px-4 rounded transition-colors disabled:opacity-50"
                >
            ...
            {loading && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {[...Array(3)].map((_, i) => (
                        <div key={i} className="bg-black border border-zinc-800 p-6 shadow-lg animate-pulse">
                            ...
                            <div className="h-4 bg-zinc-800 rounded w-1/3 mt-6 ml-auto"></div>
                        </div>
                    ))}
                </div>
            )}
            ...
            {!loading && !error && cases.length === 0 && (
                 <div className="bg-black border border-zinc-800 p-8 text-center text-zinc-500">
